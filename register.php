...
<html lang="pt-br">
        ...
        <section class="showcase">
            ...
            <div class="container">
                <section id="login">
                    <h1>Cadastro</h1>
                    <form method="POST">
                        <input type="text" name="name" placeholder="Nome Completo" autofocus maxlength="30">
                        <input type="tel" name="telephone" placeholder="Telefone" maxlength="30">
                        ...
                        <input type="password" name="confirmpassword" placeholder="Confirmar Senha" maxlength="16">
                        <a href="">Cadastrar</a>
                    </form>
                </section>
            </div>
    ...
    <?php
    // VERIFICAR se houve CLIQUE no botão CADASTRAR.
    if(isset($_POST['name']))

    // O isset VERIFICA a existência de uma VARIÁVEL, de um ARRAY...
    
    // Nesse caso foi através no Método POST.
    {
        ...
        // addslashes evita CÓDIGOS MALICIOSOS dentro do FORMULÁRIO.

        // VERIFICAR se está PREENCHIDO todos os CAMPOS.
        if(!empty($name) && !empty($telephone) && !empty(email) && 
        !empty(password) && !empty(confirmpassword)) 
        {
            // Tudo PREENCHIDO, enviar os dados para a CLASSE, no Método CADASTRAR.
            // Para usar o Método CADASTRAR, é preciso INSTANCIAR a CLASSE.
            // E em php, é necessário TRAZER o DOCUMENTO para a PÁGINA.

            // Primeiramente, ACESSANDO o Método CONECTAR, com seus PARÂMETROS.
            $u->connect("Helthier","192.168.0.168", "root","");

            // VERIFICAR se não ocorreu algum ERRO.
            if($u->msgErro == "") 
            // Se estiver VAZIA, está OK.
            {
                // VERIFICAR se SENHA é IGUAL a CONFIRMAR SENHA.
                if($password == $confirmpassword) 
                {
                    // Se for IGUAL, mandar CADASTRAR.
                    if($u->register($name,$telephone,$email,$password)) 
                    {
            ...
            {
                // Caso estiver PREENCHIDA, tem um ERRO.
